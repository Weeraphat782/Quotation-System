...
export default function QuotationPreview({ quotation, onClose }: QuotationPreviewProps) {
  ...
  const page1Items = allItems.filter(item => (item.page || 1) === 1)
  const page2Items = allItems.filter(item => (item.page || 1) === 2)
  ...
  const renderItemDescription = (item: QuotationItem) => (
  ...
  const renderItemsTable = (items: QuotationItem[], showHeader: boolean) => (
        ...
        {items.map((item) => (
          <tr key={item.id}>
            ...
            <td className="text-right py-2 px-2 align-top font-mono">
              {item.hide_price ? "" : item.price.toLocaleString("en-US", { minimumFractionDigits: 2 })}
            </td>
          </tr>
        ))}
      </tbody>
  ...
  const renderPage = (pageNum: number) => {
    ...
    const items = pageNum === 1 ? page1Items : page2Items
    ...
    return (
      ...
      >
        ...
        {isLastPage && renderFooterSection()}

        {/* Page footer with address and page number */}
  ...
  return (
    <div className="bg-card min-h-screen">
      ...
      {/* Document Pages */}
      <div className="print-page-wrapper py-8 px-4 bg-slate-100 min-h-screen flex flex-col items-center gap-8 print:p-0 print:bg-white print:gap-0">
        {renderPage(1)}
        {hasPage2 && renderPage(2)}
      </div>
    </div>
  )
}
